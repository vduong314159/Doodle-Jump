...
/**
 * I filled out this class and it looks like the code in lecture. 
 * I added a setTransparency method.
 * 
...
public abstract class Shape implements cs015.prj.Shape.ShapeInterface {
	...
	public double getRotation() {
		_rotationAngle = _rotationAngle*180/Math.PI;
	...
	public void setLocation(double x, double y) {
		if (_wrapping) {
			double _wrapX = ((x % _container.getWidth()) + _container.getWidth()) % _container.getWidth();
			x = _wrapX;

			double _wrapY = ((y % _container.getHeight()) + _container.getHeight()) % _container.getHeight();
			y = _wrapY;
		}
	...
	public void setTransparency(int alpha) {
		_borderColor = new java.awt.Color(_borderColor.getRed(), _borderColor.getGreen(), _borderColor.getBlue(), alpha);
		_fillColor = new java.awt.Color(_fillColor.getRed(), _fillColor.getGreen(), _fillColor.getBlue(), alpha);
	}

	public void antiAlias(java.awt.Graphics2D brush) {
		java.awt.RenderingHints _antiAliasIt = new java.awt.RenderingHints(java.awt.RenderingHints.KEY_ANTIALIASING, java.awt.RenderingHints.VALUE_ANTIALIAS_ON);
		brush.setRenderingHints(_antiAliasIt);
	}
